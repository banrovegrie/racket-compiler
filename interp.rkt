...
#;(define interp-R3-prime (lambda (p) ((send (new interp-R3-class) interp-scheme '()) p)))

(define interp-F1 (lambda (p) ((send (new interp-R4-class) interp-F '()) p)))

(define interp-F2 (lambda (p) ((send (new interp-R5-class) interp-F '()) p)))

(define interp-F3 (lambda (p) ((send (new interp-R6-class-alt) interp-F '()) p)))

(define interp-F4 (lambda (p) ((send (new interp-R8-class) interp-F '()) p)))

;; Interpreters for C2 and C3.

#;(define interp-C2 (lambda (p) (send (new interp-R3-class) interp-C p)))

#;(define interp-C3 (lambda (p) (send (new interp-R4-class) interp-C p)))

#;(define interp-C4 (lambda (p) (send (new interp-R5-class) interp-C p)))

#;(define interp-C5 (lambda (p) (send (new interp-R6-class-alt) interp-C p)))

#;(define interp-C7 (lambda (p) (send (new interp-R8-class) interp-C p)))

;; Interpreters for x86 with names that correspond to the book.

;; TODO: update the following names!

(define interp-pseudo-x86-0 (lambda (p) ((send (new interp-R1-class) interp-pseudo-x86 '()) p)))

(define interp-x86-0 (lambda (p) ((send (new interp-R1-class) interp-x86 '()) p)))

(define interp-pseudo-x86-1 (lambda (p) ((send (new interp-R2-class) interp-pseudo-x86 '()) p)))

(define interp-x86-1 (lambda (p) ((send (new interp-R2-class) interp-x86 '()) p)))

(define interp-pseudo-x86-2 (lambda (p) ((send (new interp-R3-class) interp-pseudo-x86 '()) p)))
...
(define interp-x86-2 (lambda (p) ((send (new interp-R3-class) interp-x86 '()) p)))

(define interp-pseudo-x86-3 (lambda (p) ((send (new interp-R4-class) interp-pseudo-x86 '()) p)))
...
(define interp-x86-3 (lambda (p) ((send (new interp-R4-class) interp-x86 '()) p)))

(define interp-pseudo-x86-4 (lambda (p) ((send (new interp-R6-class-alt) interp-pseudo-x86 '()) p)))

(define interp-x86-4 (lambda (p) ((send (new interp-R6-class-alt) interp-x86 '()) p)))

(define interp-pseudo-x86-5 (lambda (p) ((send (new interp-gradual-class) interp-pseudo-x86 '()) p)))

(define interp-x86-5 (lambda (p) ((send (new interp-gradual-class) interp-x86 '()) p)))
...
(define interp-R1-class
  (class object%
    ...
    (define/public (return-from-tail v env) (cons (cons result v) env))
    ...
    (define/public (primitives) (set '+ '- 'read))
    ...
    (define/public (interp-scheme-exp env)
      (lambda (ast)
        ...
        (match ast
          ...
          [(Prim op args) (apply (interp-op op)
                                 (for/list ([e args])
                                   (recur e)))]
          [else (error (format "R1/no match in interp-scheme-exp for ~a" ast))])))

    (define/public (interp-scheme env)
      (lambda (ast)
        (verbose "R1/interp-scheme" ast)
        (match ast
          [(Program _ e) ((interp-scheme-exp '()) e)]
          [else (error (format "R1/no match in interp-scheme for ~a" ast))])))
      ...
      (lambda (ast)
        (define result
          (match ast
            [(Var x) (lookup x env)]
            [(Int n) n]
            [(Prim op args) (apply (interp-op op) (map (interp-C-exp env) args))]
            [else (error "C0/interp-C-exp unhandled" ast)]))
    ...
    (define/public (get-name ast)
      (match ast
        [(or (Var x) (Reg x)) x]
        [(Deref 'rbp n) n]
        [else (error 'interp-R1-class/get-name "doesn't have a name: ~a" ast)]))

    (field [x86-ops
            (make-immutable-hash
             `((addq 2 ,+) (imulq 2 ,*) (subq 2 ,(lambda (s d) (- d s))) (negq 1 ,-)))])
    ...
    (define/public (interp-x86-exp env)
      (lambda (ast)
        ...
        (define result
          (match ast
            [(or (Var x) (Reg x)) (lookup (get-name ast) env)]
            [(Deref r n) (lookup (get-name ast) env)]
            [(Imm n) n]
            [else (error 'interp-R1-class/interp-x86-exp "unhandled ~a" ast)]))
    ...
    (define/public (interp-x86-instr env)
      (lambda (ast)
        ...
        (match ast
          ['() env]
          [(cons (Callq 'read_int _) ss) (let ([v (read)])
                                           (copious "read " v)
          ...
          [(cons (Jmp conclusion) ss)
           ...
           env]
          [(cons (Jmp label) ss) ((interp-x86-block env) (goto-label label))]
          [(X86Program info ss) (let ([env ((interp-x86-instr '()) ss)]) (lookup 'rax env))]
          [(cons (Instr binary-op (list s d)) ss) (let ([s ((interp-x86-exp env) s)]
          ...
          [(cons (Instr unary-op (list d)) ss)
           (let ([d ((interp-x86-exp env) d)] [x (get-name d)] [f (interp-x86-op unary-op)])
             (let ([v (f d)])
               (copious "unary-op result " (observe-value v))
               ((interp-x86-instr (cons (cons x v) env)) ss)))]
          [else (error "R1/interp-x86-instr no match for" ast)])))

    (define/public (interp-pseudo-x86 env) (lambda (ast) ((interp-x86 env) ast)))

    (define/public (interp-x86-block env)
      (lambda (ast)
        (match ast
          [(Block info ss) ((interp-x86-instr env) ss)]
          [else (error "R1/interp-x86-block unhandled" ast)])))
    ...
    (define/public (interp-x86 env)
      (lambda (ast)
        ...
        (match ast
          [(X86Program info blocks) (parameterize ([get-basic-blocks blocks])
                                      (define start-block (dict-ref blocks 'start))
                                      (define result-env ((interp-x86-block '()) start-block))
                                      (lookup 'rax result-env))]
          [else (error "R1/interp-x86 no match in for" ast)]))))) ;; class interp-R1-class
...
(define interp-R2-class
  (class interp-R1-class
    ...
    ;; -Jeremy
    (define/override (primitives) (set-union (super primitives) (set 'eq? 'not 'or '< '<= '> '>=)))

    (define/override (interp-op op)
      (match op
        ['eq? (lambda (v1 v2)
                (cond
                  [(and (fixnum? v1) (fixnum? v2)) (eq? v1 v2)]
    ...
    (define/override (interp-scheme-exp env)
      (lambda (ast)
        ...
        (match ast
          [(HasType e t) (recur e)]
          [(Bool b) b]
          [(Prim 'and (list e1 e2)) (match (recur e1)
                                      [#t (match (recur e2)
                                            [#t #t]
                                            [#f #f])]
                                      [#f #f])]
          [(If cnd thn els) (match (recur cnd)
                              [#t (recur thn)]
                              [#f (recur els)]
                              [else (error 'interp-scheme-exp "R2 expected Boolean, not ~a" cnd)])]
    ...
    (define/override (interp-C-tail env)
      (lambda (ast)
        ...
        (match ast
          [(IfStmt cnd thn els)
           (if ((interp-C-exp env) cnd) ((interp-C-tail env) thn) ((interp-C-tail env) els))]
          [(Goto label) ((interp-C-tail env) (goto-label label))]
          [else ((super interp-C-tail env) ast)])))

    (define/override (interp-C ast)
      (copious "R2/interp-C" ast)
      (match ast
        [(CProgram info blocks) (parameterize ([get-basic-blocks blocks])
    ...
    (define/override (get-name ast)
      (match ast
        [(ByteReg r) (super get-name (Reg (byte2full-reg r)))]
    ...
    (set! x86-ops
          (hash-set* x86-ops 'notq `(1 ,bitwise-not) 'andq `(2 ,bitwise-and) 'xorq `(2 ,bitwise-xor)))
    ...
    (define/override (interp-x86-exp env)
      (lambda (ast)
        ...
        (define result
          (match ast
            [(ByteReg r) ((interp-x86-exp env) (Reg (byte2full-reg r)))]
            [(Bool #t) 1]
            [(Bool #f) 0]
            [(Prim 'eq? (list e1 e2))
             (if (eq? ((interp-x86-exp env) e1) ((interp-x86-exp env) e2)) 1 0)]
            [(Prim '< (list e1 e2)) (if (< ((interp-x86-exp env) e1) ((interp-x86-exp env) e2)) 1 0)]
            [(Prim '<= (list e1 e2))
             (if (<= ((interp-x86-exp env) e1) ((interp-x86-exp env) e2)) 1 0)]
            [(Prim '> (list e1 e2)) (if (> ((interp-x86-exp env) e1) ((interp-x86-exp env) e2)) 1 0)]
            [(Prim '>= (list e1 e2))
             (if (>= ((interp-x86-exp env) e1) ((interp-x86-exp env) e2)) 1 0)]
    ...
    (define (eflags-status env cc)
      ...
      (match cc
        ...
        ['le (if (or (eq? 1 (eflags-status env 'e)) (eq? 1 (eflags-status env 'l))) 1 0)]
        ['g (if (not (eq? 1 (eflags-status env 'le))) 1 0)]
        ['ge (if (not (eq? 1 (eflags-status env 'l))) 1 0)]))
    ...
    (define/override (interp-x86-instr env)
      (lambda (ast)
        ...
        (match ast
          [(Block `(lives ,lives) ss) ((interp-x86-instr env) ss)]
          [(Block `(lives ,lives) ss) ((interp-x86-instr env) ss)]
          ...
          [(cons (IfStmt cnd thn els) ss) (if (not (eq? 0 ((interp-x86-exp env) cnd)))
          ...
          [(cons (Instr 'movzbq (list s d)) ss)
           ...
           (define v ((interp-x86-exp env) s))
           ((interp-x86-instr (cons (cons x v) env)) ss)]
          [(cons (JmpIf cc label) ss) (cond
                                        [(eq? (eflags-status env cc) 1)
                                         ((interp-x86-block env) (goto-label label))]
                                        [else ((interp-x86-instr env) ss)])]
          [else ((super interp-x86-instr env) ast)]))))) ;; class interp-R2-class
...
(define interp-R3-class
  (class interp-R2-class
        ...
        (let-values ([(start stop name vect) (fetch-page addr)])
          (let ([value (vector-ref vect (arithmetic-shift (- addr start) -3))])
            (when (equal? value uninitialized)
              (error 'interp-R3-class/memory-read "read uninitialized memory at address ~s" addr))
            value))))
    ...
    (define (allocate-page! name size)
      ...
      (unless (and (fixnum? size) (positive? size) (= 0 (modulo size 8)))
        (error 'allocate-page! "expected non-negative fixnum in ~a" size))
      ;; Find the last address
      (define max-addr
        (for/fold ([next 8]) ([page (in-list (unbox memory))])
          (match-let ([`(page ,_ ,stop ,_ ,_) page]) (max next stop))))
      ...
      (set-box! memory (cons `(page ,start-addr ,stop-addr ,name ,vect) (unbox memory)))
      start-addr)
                ...
                (let loop ([memory (unbox memory)])
                  (match memory
                    ...
                    [`(,(and page `(page ,ptr ,_ ,_ ,_)) . ,pages)
                     (if (= addr ptr) pages (cons page (loop pages)))]))))
      ...
      (let search ([m (unbox memory)])
        (match m
          ...
          [`((page ,min ,max ,name ,vect) . ,rest-memory)
           ...
           (if (and (<= min addr) (< addr max)) (values min max name vect) (search rest-memory))]
    ...
    #;(define/public (scheme-vector-ref vec i)
        (match vec
          [`(vector-proxy ,v ,rs ,ws)
           ...
           (apply-fun (lambda (env) (interp-scheme-exp env)) r (list v^))]
    ...
    #;(define/public (scheme-vector-set! vec i arg)
        (match vec
          [`(vector-proxy ,v ,rs ,ws)
           ...
           (define arg^ (apply-fun (lambda (env) (interp-scheme-exp env)) w (list arg)))
    ...
    (define/override (interp-scheme-exp env)
      (lambda (ast)
        ...
        (match ast
          [(Void) (void)]
          [(GlobalValue 'free_ptr) (unbox free_ptr)]
          [(GlobalValue 'fromspace_end) (unbox fromspace_end)]
    ...
    (define/override (interp-scheme env)
      (lambda (ast)
        ...
        (match ast
          [(Program info e)
           ((initialize!) runtime-config:rootstack-size runtime-config:heap-size)
           ((interp-scheme-exp '()) e)]
          [else ((super interp-scheme env) ast)])))

    (define (mem-error message expr)
      (lambda (who fmt . args)
        (error who "~a in ~a raise error:\n~a" message expr (apply format (cons fmt args)))))

    (define (global-value-err ast)
      (lambda () (error 'interp-R3-class "global label is unknown in ~a" ast)))
    ...
    (define/public (fetch-global label)
      (let* ([err (global-value-err label)]
        ...
        (when (equal? value uninitialized)
          ...
          (error 'interp-R3-class/fetch-global "global value, ~a, used before initialization" label))
        value))
        ...
        (define result
          (match ast
            ...
            [(CollectionNeeded? size)
             (when (or (eq? (unbox free_ptr) uninitialized) (eq? (unbox fromspace_end) uninitialized))
            ...
            [(AllocateClosure l ty arity)
             ...
             (vector-set! vec l `(arity ,arity))
             vec]
            #;[(AllocateClosure l ty arity) (build-vector l (lambda a uninitialized))]
            [(AllocateProxy ty) (build-vector 3 (lambda a uninitialized))]
            [else ((super interp-C-exp env) ast)]))
    ...
    (define (x86-binary-op? x)
      (let ([val (hash-ref x86-ops x #f)]) (and val (= (car val) 2))))

    (define (x86-unary-op? x)
      (let ([val (hash-ref x86-ops x #f)]) (and val (= (car val) 1))))
      ...
      (lambda (ast)
        ...
        (match ast
          #;[(cons (Callq 'malloc) ss)
             ...
             ((interp-x86-instr `((rax . ,(allocate-page! 'malloc num-bytes)) . ,env)) ss)]
          #;[(cons (Callq 'alloc) ss)
             (define num-bytes ((interp-x86-exp env) (Reg 'rdi)))
             ((interp-x86-instr `((rax . ,(allocate-page! 'alloc num-bytes)) . ,env)) ss)]
        ...
        (match ast
          [(X86Program info blocks)
           ...
           (parameterize ([get-basic-blocks blocks])
             (let ([env^ ((interp-x86-block env) (dict-ref blocks 'start))]) (lookup 'rax env^)))])))
    ...
    (set!
     x86-ops
     (hash-set* x86-ops 'sarq `(2 ,(lambda (n v) (arithmetic-shift v (- n)))))))) ;; interp-R3-class
...
(define interp-R4-class
  (class interp-R3-class
    ...
    (define/public (non-apply-ast)
      (set-union (primitives) (set 'if 'let 'define 'program 'has-type 'void)))

    (define/public (interp-scheme-def d)
      (match d
        [(Def f `([,xs : ,ps] ...) rt info body) (mcons f `(function ,xs ,body ()))]))
    ...
    (define/override (interp-scheme env)
      (lambda (ast)
        (verbose "R4/interp-scheme" ast)
        (match ast
          #;[`(program (type ,ty) ,ds ... ,body) ((interp-scheme '()) `(program ,@ds ,body))]
          [(Program info (cons ds body))
           ((initialize!) runtime-config:rootstack-size runtime-config:heap-size)
           (let ([top-level (for/list ([d ds])
                              (interp-scheme-def d))])
             (for ([b top-level])
               (set-mcdr! b
                          (match (mcdr b)
                            [`(function ,xs ,body ()) `(function ,xs ,body ,top-level)])))
    ...
    #;(define/public (F-vector-ref vec i)
        (match vec
          [`(vector-proxy ,v ,rs ,ws)
           ...
           (define r (vector-ref rs i))
           (apply-fun (lambda (env) (interp-F env)) r (list v^))]
          [else (vector-ref vec i)]))
    ...
    #;(define/public (F-vector-set! vec i arg)
        (match vec
          [`(vector-proxy ,v ,rs ,ws)
           (define w (vector-ref ws i))
           (define arg^ (apply-fun (lambda (env) (interp-F env)) w (list arg)))
    ...
    (define/public (interp-C-def ast)
      ...
      (match ast
        [(Def f `([,xs : ,ps] ...) rt info blocks) (mcons f `(function ,xs ((name . ,f)) ,blocks ()))]
        [else (error "R4/interp-C-def unhandled" ast)]))
      ...
      (match ast
        [(ProgramDefs info ds)
         ...
         (for/list ([b top-level])
           (set-mcdr! b
                      (match (mcdr b)
                        [`(function ,xs ,info ,blocks ()) `(function ,xs ,info ,blocks ,top-level)])))
         ((interp-C-tail top-level) (TailCall (Var 'main) '()))]
        [else (error "R4/interp-C unhandled" ast)]))

    (define (stack-arg-name n)
      (string->symbol (string-append "rsp_" (number->string n))))

    (define/public (builtin-funs) (set 'malloc 'alloc 'collect 'initialize 'read_int 'exit))
    ...
    (define/override (observe-value v)
      (match v
        [`(function ,info ,blocks ,def-env) `(function ,(dict-ref info 'name))]
        [else v]))
         ...
         (define passing-regs
           (filter (lambda (p) p)
                   (for/list ([r (vector-take arg-registers n)])
                     (let ([v (lookup r env #f)]) (if v (cons r v) #f)))))
         (debug "interp-x86 call-function" passing-regs)
         (define variable-size 8) ;; ugh -Jeremy
         (define root-size
           (cond
             [root-spills (* variable-size root-spills)]
             [else 0]))
         (set! root-stack-pointer (+ root-stack-pointer root-size))
         (define new-env (cons (cons 'r15 root-stack-pointer) (append passing-regs def-env)))
         ;; interpret the body of the function in the new-env
         (define result-env
           (parameterize ([get-basic-blocks blocks])
             ((interp-x86-block new-env) (dict-ref blocks (symbol-append f 'start)))))
         (set! root-stack-pointer (- root-stack-pointer root-size))
    ...
    #;(define (apply-vector-ref vec i cont-ss env)
        ...
        (cond
          [(equal? (arithmetic-shift tag -63) 1)
    ...
    (define/override (interp-x86-instr env)
      (lambda (ast)
        ...
        (match ast
          ...
          [(cons (Callq 'exit _) ss) (error 'interp-x86 "exiting")]
          [else ((super interp-x86-instr env) ast)])))
      ...
      (lambda (ast)
        ...
        (match ast
          [(ProgramDefs info ds)
           ...
           (define env^ '())
           (define result-env (call-function (lookup 'main top-level) '() env^))
           (lookup 'rax result-env)]))))) ;; end  interp-R4-class
...
(define interp-R5-class
  (class interp-R4-class
    ...
    (define/override (primitives) (set-union (super primitives) (set 'procedure-arity)))

    (define/override (non-apply-ast)
      (set-union (super non-apply-ast) (set 'global-value 'allocate 'collect)))
      ...
      (match op
        ['procedure-arity (lambda (v)
                            (match v
                              ...
                              [(vector `(function ,ps ,rst ...) vs ... `(arity ,n)) n]
                              [else (error 'interp-op "expected function, not ~a" v)]))]
        [else (super interp-op op)]))
    ...
    (define/override (interp-scheme-exp env)
      (lambda (ast)
        ...
        (match ast
          [(Lambda `([,xs : ,Ts] ...) rT body) `(function ,xs ,body ,env)]
          [else ((super interp-scheme-exp env) ast)])))
...
(define (crop-to-64bits n)
  (let ([s (string->list (number->string n 2))])
    (cond
      [(> (length s) 64) (let ([s^ (list-tail s (- (length s) 64))])
...
(define interp-R6-class-alt
  (class interp-R5-class
    ...
    (define/override (primitives)
      (set-union
       (super primitives)
       (set 'boolean? 'integer? 'vector? 'procedure? 'tag-of-any 'value-of-any 'tag-of-vector)))
    ...
    (define/override (interp-op op)
      (match op
        ...
        ['eq? (lambda (v1 v2)
                (match (list v1 v2)
                  [`((tagged ,v1^ ,ty1) (tagged ,v2^ ,ty2)) (and (eq? v1^ v2^) (equal? ty1 ty2))]
                  [else (cond
                          [(or (and (fixnum? v1) (fixnum? v2))
                               (and (boolean? v1) (boolean? v2))
                               (and (vector? v1) (vector? v2))
        ...
        ['tag-of-any (lambda (v)
                       (match v
                         [`(tagged ,v^ ,tg) tg]
                         [else (error "interp expected tagged value, not" v)]))]
        ...
        ['any-vector-ref (lambda (v i)
                           (match v
                             [`(tagged ,v^ ,tg) (vector-ref v^ i)]))]
        ['any-vector-set! (lambda (v i a)
                            (match v
                              [`(tagged ,v^ ,tg) (vector-set! v^ i a)]))]
        ['any-vector-length (lambda (v)
                              (match v
                                [`(tagged ,v^ ,tg) (vector-length v^)]))]
    ...
    (define/public (tyeq? t1 t2)
      (match `(,t1 ,t2)
        [`((Vectorof Any) (Vector ,t2s ...)) (for/and ([t2 t2s])
                                               (eq? t2 'Any))]
        [`((Vector ,t1s ...) (Vectorof Any)) (for/and ([t1 t1s])
                                               (eq? t1 'Any))]
        [else (equal? t1 t2)]))
    ...
    (define/override (interp-C-exp env)
      (lambda (ast)
        (define result
          (match ast
            #;[(Inject e t) `(tagged ,((interp-C-exp env) e) ,t)]
            #;[(Project e t2)
               ...
               (match v
                 [`(tagged ,v1 ,t1) (cond
                                      ...
                                      [else (error "in project, type mismatch" t1 t2)])]
                 [else (error "in project, expected injected value" v)])]
            [(ValueOf e ty) ((interp-op 'value-of-any) ((interp-C-exp env) e))]
            [(Exit) (error 'interp-C-exp "exiting")]
            [else ((super interp-C-exp env) ast)]))
        ...
        (match ty
          ['Any
           ...
           (cond
             ...
             [(eq? tag 4) ;; boolean
              (if (eq? 0 (arithmetic-shift val (- 3))) `(tagged #f Boolean) `(tagged #t Boolean))]
             ;; vector
             ;; This needs work. I need to find out how to get the
             ;; length of the vector from in memory. -Jeremy
             [(eq? tag 2) `(tagged vector (Vectorof Any))]
             ;; procedure (represented by a closure)
             [(eq? tag 3) `(tagged procedure ,ty)]
             [(eq? tag 5) ;; void
              `(tagged void Void)])]
    ...
    (set! x86-ops
          (hash-set* x86-ops
                     ...
                     'salq
                     `(2 ,(lambda (n v)
                            (crop-to-64bits (arithmetic-shift v n)))))))) ;; interp-R6-class-alt
  ...
  (class interp-R6-class-alt
    ...
    (define/override (apply-fun interp fun-val arg-vals)
      (match fun-val
        ...
        [`(function (,xs ...) ,body ,lam-env)
         (define new-env
           (append (for/list ([x xs] [arg arg-vals])
                     (cons x (box arg)))
                   lam-env))
         ((interp new-env) body)]
        [else (error 'apply-fun "expected function, not ~a" fun-val)]))
      ...
      (lambda (ast)
        ...
        (match ast
          [(Call f args)
           ((interp-C-exp env) ast)
           env]
          [else ((super interp-C-stmt env) ast)]))))) ;; interp-R8-class
